...
const AdminTripUpdate = () => {
  ...
  useEffect(() => {
    const fetchTripDetails = async () => {
      if (!id) return;
      try {
        const res = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/trips/${id}/details`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/admin/trips/${id}`, {
        method: 'PUT',
        headers: {
